...
import { getJsonValueType, jsonValueEquals, matchesType, } from "./internal.js";
export function isValueValidAgainstSchema(schema, value, root) {
    return evaluateSchema(schema, value, root, { collectAll: false }).valid;
}
export function evaluateSchema(schema, value, root, options = {}) {
    const state = {
        root,
        issues: [],
...
function validateSchema(schema, value, path, state) {
...
function validateComposition(schema, value, path, state) {
    if (schema.allOf) {
    ...
    if (schema.oneOf?.length) {
        const matchCount = schema.oneOf.filter((branch) => validateSilently(branch, value, state.root)).length;
        if (matchCount !== 1) {
...
function validateObjectKeywords(schema, value, path, state) {
    if (!isJsonObject(value)) {
        return;
    }
    for (const property of schema.required ?? []) {
        if (!(property in value)) {
            addIssue(state, "required", appendPointer(path, property), "Required property is missing.");
        }
    }
    ...
    for (const [dependency, requiredProperties] of Object.entries(schema.dependentRequired ?? {})) {
        ...
        for (const property of requiredProperties) {
            if (!(property in value)) {
                addIssue(state, "dependentRequired", appendPointer(path, property), `Required when ${dependency} is present.`);
            }
        }
    }
...
function validateArrayKeywords(schema, value, path, state) {
...
function validateStringKeywords(schema, value, path, state) {
...
function validateNumberKeywords(schema, value, path, state) {
...
function validateSilently(schema, value, root) {
    return evaluateSchema(schema, value, root, { collectAll: false }).valid;
}
function addIssue(state, keyword, instancePath, message) {
    state.issues.push({ keyword, instancePath, message });
    if (!state.collectAll) {
        throwIfShortCircuit();
    }
}
function throwIfShortCircuit() {
    throw SHORT_CIRCUIT;
}
...
function matchesPatternProperty(schema, property) {
