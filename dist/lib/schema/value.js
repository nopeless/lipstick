...
export function buildInitialValue(schema, root) {
    ...
    if (isObjectSchema(resolved)) {
        ...
        for (const [key, child] of Object.entries(resolved.properties ?? {})) {
            if (required.has(key) || child.default !== undefined || child.const !== undefined) {
...
export function sanitizeValueForSchema(value, schema, root) {
    const resolved = resolveSchema(schema, root, value);
    if (resolved.const !== undefined) {
        return resolved.const;
    }
    if (resolved.enum?.length) {
        return resolved.enum.includes(value) ? value : resolved.enum[0];
    }
    if (resolved.oneOf?.length) {
    ...
    if (isObjectSchema(resolved)) {
        ...
        for (const [key, child] of Object.entries(resolved.properties ?? {})) {
            ...
            else if (required.has(key) || child.default !== undefined || child.const !== undefined) {
                next[key] = buildInitialValue(child, root);
            }
        }
        for (const [key, childValue] of Object.entries(source)) {
            ...
            if (typeof resolved.additionalProperties === "object" &&
                ...
                next[key] = sanitizeValueForSchema(childValue, resolved.additionalProperties, root);
                continue;
            }
    ...
    if (isArraySchema(resolved)) {
        ...
        const next = source.map((item, index) => sanitizeValueForSchema(item, getArrayItemSchema(resolved, index) ?? {}, root));
